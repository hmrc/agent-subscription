...
class CompaniesHouseControllerISpec
extends BaseISpec
...
with AuthStub {
  ...
  "GET of /companies-house-api-proxy/company/:crn/officers/:name" should {
    "return a 401 when the user is not authenticated" in {
      ...
      val response = new Resource(s"/agent-subscription/companies-house-api-proxy/company/$crn/officers/$name", port).get()
    ...
    "return NotFound if the name does not appear on the officers list" in {
      ...
      val response = new Resource(s"/agent-subscription/companies-house-api-proxy/company/${crn.value}/officers/BROWN", port).get()
    ...
    "return BadRequest if there is an invalid identifier" in {
      ...
      val response = new Resource(s"/agent-subscription/companies-house-api-proxy/company/NOT-VALID/officers/FERGUSON", port).get()
    ...
    "return NotFound if the Companies House API token has expired" in {
      ...
      val response = new Resource(s"/agent-subscription/companies-house-api-proxy/company/SC123456/officers/FERGUSON", port).get()
      response.status shouldBe 404
    }
  ...
  "GET of /companies-house-api-proxy/company/:crn/status" should {
    "return a 401 when the user is not authenticated" in {
      requestIsNotAuthenticated()
      val response = new Resource(s"/agent-subscription/companies-house-api-proxy/company/${crn.value}/status", port).get()
      response.status shouldBe 401
    }

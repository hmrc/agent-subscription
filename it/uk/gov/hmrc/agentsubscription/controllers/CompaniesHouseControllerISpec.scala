...
class CompaniesHouseControllerISpec extends BaseISpec with CompaniesHouseStub with AuthStub {
  ...
  "GET of /companies-house-api-proxy/company/:crn/officers/:name" should {
    ...
    "return NotFound if the name does not appear on the officers list" in {
      ...
      val response = await(new Resource(s"/agent-subscription/companies-house-api-proxy/company/SC123456/officers/no-match", port).get)
      response.status shouldBe 404
    }

    "return NotFound if the name appears on the officers list but they have retired" in {
      requestIsAuthenticatedWithNoEnrolments()
      givenCompaniesHouseOfficersListFoundForCrn(crn)
    ...
    "return BadRequest if there is an invalid identifier" in {
      requestIsAuthenticatedWithNoEnrolments()
      givenCompaniesHouseOfficersListWithStatus("NOT-VALID", 400)
      val response = await(new Resource(s"/agent-subscription/companies-house-api-proxy/company/NOT-VALID/officers/no-match", port).get)
    ...
    "return OK if a match was found" in {
      requestIsAuthenticatedWithNoEnrolments()
      givenCompaniesHouseOfficersListFoundForCrn(Crn("SC123456"))
      val response = await(new Resource(s"/agent-subscription/companies-house-api-proxy/company/SC123456/officers/lucas", port).get)

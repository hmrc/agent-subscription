...
class WhitelistISpec extends UnitSpec with OneServerPerSuite {
  ...
  protected def additionalConfiguration = Map(
  ...
  def authResponseFor(arn: Arn, trueClientIp: Option[String]): HttpResponse =
    new Resource(s"/agent-subscription/sandbox/agencies/subscriptions/none", port)

...
@Singleton
class MappingConnector @Inject() (appConfig: AppConfig, http: HttpClient, metrics: Metrics) extends Logging {
  ...
  def createMappings(arn: Arn)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Unit] = {
    ...
    val timer = metrics.defaultRegistry.timer("ConsumedAPI-Mapping-CreateMappings-PUT")
    timer.time()
    http
  ...
  def createMappingDetails(arn: Arn)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Unit] = {
    ...
    val timer = metrics.defaultRegistry.timer("ConsumedAPI-Mapping-createOrUpdateMappingDetails-POST")
    timer.time()

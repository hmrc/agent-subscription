...
class SubscriptionJourneyControllerSpec extends UnitSpec with Results with MockitoSugar {
  ...
  "Subscription Journey Controller" should {
    ...
    "return 200 with authProviderID, cleanCredsAuthProviderID and businessDetails updated when there is already an existing record" in {
      ...
      val newBusinessDetails = BusinessDetails(BusinessType.LimitedCompany, Utr("12345"), Postcode("BN65GJ"), None, None, None, None, None, None)
      val newRecord = minimalRecord.copy(authProviderId = newAuthProviderId, cleanCredsAuthProviderId = Some(newAuthProviderId), businessDetails = newBusinessDetails) // The new record (that we're trying to store)
      val updatedExistingRecord = existingRecord.copy(authProviderId = newAuthProviderId, cleanCredsAuthProviderId = Some(newAuthProviderId), businessDetails = newBusinessDetails) // The existing record, modified with the new record's authId and clean credID
